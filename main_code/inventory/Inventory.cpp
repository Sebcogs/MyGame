...
{  //Check if exists already, if it does increase the potion number 
	string addedName = thing.getName();
	int qty = thing.getNumber();
//cout<<" name and qty "<<addedName<<","<<qty<<endl;
	bool inInv = false;
	for(int i = 0; i < getPotionNum(); ++i)
		{
		if(addedName == potions[i].getName())
			{
		//cout<<"already have some"<<endl;
...
int Inventory::getArmorNum() const
{
	return armor.size();
}
int Inventory::getWeapNum() const
{
	return weapons.size();
}
...
int Inventory::getThrownNum() const
{
	return thrown.size();
}
int Inventory::getPotionNum() const
{
	return potions.size();
}
int Inventory::getAmmoNum() const
{
	return ammo.size();
}
...
int Inventory::getCopper() const
{
	return copper;
}
int Inventory::getSilver() const
{
	return silver;
}
