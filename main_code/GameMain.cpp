...
int rounds = 1;
        while (! finished && alive) 
			{
				alive = false;       //check if all are dead
				for(int x=0; x< heroNumber; x++)
						{
						if(champ[x].aliveOrDead())
							alive = true;
						}
...
cout <<"-----Round "<<rounds<<"-----"<<endl;
	...
	//find initiative for party here
				for(int x=0; x< heroNumber; x++)
						{       //heros act here 
                        if(!champ[x].condition.getIsHelpless())
                        {   //check helpless
	...
	finished = processFightCommand(ourParty, champ, parser, gamerooms, myCommand, thisRoom, x);
				...
				if(finished || myCommand.getCommandWord() == FLEE || killed == true)
					...
					if(killed)  //when all monsters are defeated, give exp
							{
					...
					int exp = totalExp/heroNumber;
						for(int i=0; i< heroNumber; i++)
                            ...
                            rounds = 1;     //reset rounds to prepare for next battle
						//cin.ignore(100, '\n');
							}	//if(killed)...
                        }
                        else
                            cout<<"You are incapacitated and cannot do anything!"<<endl;
             						}	//for hero loop
                                    
                for(int x=0; x< heroNumber; x++)       //after all heros acted that could        
                    champ[x].condition.update();    //to adjust the duration of conditions, is this the correct place for this?
		...
		//cycle thru the monsters here
				int numberOfEnemies = gamerooms.getCurrentRoom().enemies.getNumberAppear();
						...
						for(int monster=0; monster< numberOfEnemies; monster++)
							{   //cycle thru monsters, they act here
							Monster aMonster = gamerooms.getCurrentRoom().enemies.getMonster(monster);  	
							bool aliveMonster = aMonster.aliveOrDead();
                            bool inCapacitated = aMonster.condition.getIsIncap();
                            if(!inCapacitated)
							...
							usleep(DELAY);
							bool success;
							if(nextto != -1 && aliveMonster && !inCapacitated)	
								{
								success = fight(aMonster, champ[nextto], 1);
								if(!success)
									cout <<"Error in monster fighting hero."<<endl;
								}
                                //how to update monster condition?? in fight function?
                            aMonster.condition.update();
							}	//for monsters
						thisRoom.createRoom();
							rounds++;
